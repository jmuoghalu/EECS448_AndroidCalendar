...
public class WeekView extends AppCompatActivity implements View.OnClickListener {
    public final static String DATA = ""; //the key for transferring the current date between activities
    public int[] array; //a 8 width array of integers representing the data for the current date
    //[0] = current day, [1] = month, [2] = year, [3] = month's first day,[4] = month's sunday of THIS week,[5] = month's day-length,[5] = previous month's day-length, [6] = week number
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        array = getToWeek.getIntArrayExtra(DayView.DATA);
    ...
    public void onClick(View view)
    {
        switch(view.getId())
        {
            ...
            {
                Intent goToMonth = new Intent(this, MonthView.class); //tell the Month class to display the month of the current year
            ...
            case(R.id.sunday):
            {
                //excludes first week's Sunday of eighth month
                if(array[1] != 8 || array[7] != 1) // Before August, the Sunday can't be clicked
                {
                    array[0] = array[4];
            ...
            case(R.id.monday):
            {
                if(array[7] == 1 && array[4] != 1) //first week of month, Sunday is not first day of month, means Monday might be first day of month
                {
                    if(array[4] == array[6]) //Sunday was last day of previous month
                    //then Monday is first day of this month
                    {
                        array[0] = 1;
                    }
                    else //otw, go to one day after sunday
                    {
                        array[0] = array[4]+1;
                    }
                }
                else if((array[7] == 5 && array[4]+6 > array[5]) || array[7] == 6) //Fifth week goes into next month or sixth week
                //
                {
                    if(array[4] == array[5])
                    {
            ...
            case(R.id.thursday):
            {
                if(array[1] != 5 || array[7] != 5) //After May can't be clicked
                {
                    if(array[7] == 1 && array[4] != 1)
                    {
                        if(array[4] == array[6]) //Sunday was last day of previous month
